...
int ai_model2_simulate(GameState *gameState, Player *player, int alpha, int beta, int depth)
{
    ...
    if(playerTurn==1)
    {
        ...
        for(int i=0;i<cnt;i++)
        {
            ...
            BestScore=MAX(BestScore,ai_model2_simulate(gameState,player,alpha,beta,depth-1));
            env_undo(gameState);
            alpha=MAX(alpha,BestScore);
            if(alpha>=beta+MARGIN)return BestScore;
        }
    }
    else
    {
        ...
        {
            env_play(gameState,player,vector_get(&MovesStart,i),vector_get(&MovesEnd,i));
            BestScore=MIN(BestScore,ai_model2_simulate(gameState,player,alpha,beta,depth-1));
            env_undo(gameState);
...
int ai_model2_play(GameState *gameState, Player *player)
{
    ...
    {
        env_play(gameState,player,vector_get(&MovesStart,i),vector_get(&MovesEnd,i));
        score=playerTurn*ai_model2_simulate(gameState,player,-99999999,99999999,MAXSTEP);
